...
function handleRelyAndCreateRelySort() {
    ...
    var completeTime = sd.format(new Date(), 'HH:mm:ss').yellow;
    trace([`[${completeTime}] 依赖表处理完成！`]);
    ...
    completeTime = sd.format(new Date(), 'HH:mm:ss').yellow;
    if (isLog) {
        sorts(relyList);
        result = [];
        resultString = "";
        ...
        trace(`[${completeTime}] 依赖排序完成`, "耗时", new Date().getTime() - now, "毫秒！");
    ...
    onFile = function (file) {
        ...
        if (!("" + file.path).match(/\.d\.ts/ig)) {
            ...
            if (thisCite) {
                ...
                //console.log("relyArrayCache.push(file) " + atFileIndex + " : " + file.path);
